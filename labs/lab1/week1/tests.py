...
def test_factorial_num(results_dict):
    """
    Checks that lab1.factorial_of_num() returns the correct value
    """
    print("")
    print("Testing lab1.factorial_of_num()")
    message = "----"
    try:
        result = lab1.factorial_of_num(5)
        if not isinstance(result, int):
            message += 'Did not return an integer - Fail'
            test_failed(results_dict)
        else:
            ...
            test_passed(results_dict)
            # now check if the value is correct, should be 55
            if result is not 120:
                message += '\n----Did not return the correct number - Fail'
                test_failed(results_dict)
            else:
                message += '\n----The correct value (120) was returned - Pass'
                test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.factorial_of_num - Fail'
        test_failed(results_dict)
    print(message)


def square_root(results_dict):
    """
    Checks that lab1.square_root() returns the correct value
    """
    print("")
    print("Testing lab1.square_root()")
    message = "----"
    try:
        result = lab1.square_root(64)
        if result != 8:
                message += '\n----Did not return the correct number - Fail'
                test_failed(results_dict)
        else:
                message += '\n----The correct value (8) was returned - Pass'
                test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.square_root() - Fail'
        test_failed(results_dict)
    print(message)
...
def run():
    ...
    test_factorial_num(results_dict)
    square_root(results_dict)
